...
xPrompt:	.asciiz	"Enter an X coordinate      : "
yPrompt:	.asciiz	"Enter a Y coordinate       : "
sizePrompt:	.asciiz	"Enter a size               : "
colorPrompt:	.asciiz	"Enter color (r,o,y,g,b,m,w): "
...
# convertCharacterToColor(a0: character)
# Turns 'r' into the integer value LED_RED, for
# instance
convertCharacterToColor:
	push	ra
	li	v0, 0
_convertCharacterToColorR:	
	bne	a0, 'r', _convertCharacterToColorO
	li	v0, LED_RED
	j	_convertCharacterToColorExit
_convertCharacterToColorO:
	bne	a0, 'o', _convertCharacterToColorY
	li	v0, LED_ORANGE
	j	_convertCharacterToColorExit
_convertCharacterToColorY:
	bne	a0, 'y', _convertCharacterToColorG
	li	v0, LED_YELLOW
	j	_convertCharacterToColorExit
_convertCharacterToColorG:
	bne	a0, 'g', _convertCharacterToColorB
	li	v0, LED_GREEN
	j	_convertCharacterToColorExit
_convertCharacterToColorB:
	bne	a0, 'b', _convertCharacterToColorM
	li	v0, LED_BLUE
	j	_convertCharacterToColorExit
_convertCharacterToColorM:
	bne	a0, 'm', _convertCharacterToColorP
	li	v0, LED_MAGENTA
	j	_convertCharacterToColorExit
_convertCharacterToColorP:
	bne	a0, 'p', _convertCharacterToColorW
	li	v0, LED_MAGENTA
	j	_convertCharacterToColorExit
_convertCharacterToColorW:
	bne	a0, 'w', _convertCharacterToColorExit
	li	v0, LED_WHITE
	j	_convertCharacterToColorExit

_convertCharacterToColorExit:
	pop	ra
	jr	ra
	
# drawSpiral(a0: int x, a1: int y, a2: int size, a3: int colour)
drawSpiral:
	push	ra
	push	s0
	push	s1
	...
	# Keep a counter starting at size
	move	s4, a2
	
_drawSpiralLoop:
	ble	s4, 1, _drawSpiralExit		# while(size > 1) {
	
	move	a0, s0
	move	a1, s1
	move	a2, s2
	move	a3, s3
	jal	drawHorizontalLine		# 	drawHorizontalLine(x, y, size, colour)
	add	t0, s0, s2
	sub	s0, t0, 1			# 	x = x + size - 1
	sub	s2, s2, 1			# 	size--;
	...
	move	a2, s2
	move	a3, s3
	jal	drawVerticalLine		# 	drawVerticalLine(x, y, size, colour)
	add	t0, s1, s2
	sub	s1, t0, 1			# 	y = y + size - 1
	sub	s2, s2, 1			# 	size--;
	
	sub	t0, s0, s2
	add	s0, t0, 1			# 	x = x - size + 1
	move	a0, s0
	move	a1, s1
	move	a2, s2
	move	a3, s3
...
# drawVerticalLine(a0: int x, a1: int y, a2: int size, a3: int colour)
drawVerticalLine:
	# Function Prologue
	push	ra
	push	s0
...
# for(i=0; i<size; i++) {
#   displaySetLED(x, y+i, colour);
# }

	# Go into our for loop
	li	s4, 0					# s4 = 0;
_drawVerticalLineLoop:
	bge	s4, s2, _drawVerticalLineLoopExit	# while(s4 < size) {
	
	# Calculate x-coordinate is fixed
	move	a0, s0					# 	a0 = x;
	# Calculate y-coordinate = y+i
	add	a1, s1, s4				# 	a1 = y + s4;
	# Colour set by user
	move	a2, s3					# 	a2 = color;
	
	jal	displaySetLED				# 	displaySetLED(a0: x, a1: y + s4, a2: color);
	
	#Increment i
	add	s4, s4, 1				# 	s4++;
	j	_drawVerticalLineLoop			# }
_drawVerticalLineLoopExit:
	# Function Epilogue
	pop	s4
	pop	s3
	pop	s2
